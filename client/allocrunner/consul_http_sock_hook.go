...
func (p *httpSocketProxy) run(alloc *structs.Allocation) error {
	...
	if destAddr == "" {
		// no consul http address was configured, assume standard localhost:8500
		destAddr = "localhost:8500"
	}
	...
	// running as an unpriviliged user. Unix does not allo wsetting domain
